...
class Test_RG_Statistical():
    ...
    def test_gencov_tot_se(self):
        assert_allclose(np.nanstd(list(map(t('tot'), map(t('gencov'), self.rg)))),
                        np.nanmean(list(map(t('tot_se'), map(t('gencov'), self.rg)))), atol=0.02)

    def test_gencov_tot_se_noint(self):
        assert_allclose(np.nanstd(list(map(t('tot'), map(t('gencov'), self.rg_noint)))),
                        np.nanmean(list(map(t('tot_se'), map(t('gencov'), self.rg_noint)))), atol=0.02)

    def test_gencov_cat(self):
        assert_allclose(
            np.nanmean(list(map(t('cat'), map(t('gencov'), self.rg)))), [0, 0], atol=0.02)

    def test_gencov_cat_noint(self):
        assert_allclose(
            np.nanmean(list(map(t('cat'), map(t('gencov'), self.rg_noint)))), [0, 0], atol=0.02)

    def test_gencov_cat_se(self):
        assert_allclose(np.nanstd(list(map(t('cat'), map(t('gencov'), self.rg)))),
                        np.nanmean(list(map(t('cat_se'), map(t('gencov'), self.rg)))), atol=0.02)

    def test_gencov_cat_se_noint(self):
        assert_allclose(np.nanstd(list(map(t('cat'), map(t('gencov'), self.rg_noint)))),
                        np.nanmean(list(map(t('cat_se'), map(t('gencov'), self.rg_noint)))), atol=0.02)
    ...
    def test_gencov_int_se(self):
        assert_allclose(np.nanmean(list(map(t('intercept_se'), map(t('gencov'), self.rg)))),
                        np.nanstd(list(map(t('intercept'), map(t('gencov'), self.rg)))), atol=0.1)
    ...
    def test_hsq_int_se(self):
        assert_allclose(np.nanmean(list(map(t('intercept_se'), map(t('hsq2'), self.rg)))),
                        np.nanstd( list(map(t('intercept'), map(t('hsq2'), self.rg)))), atol=0.1)
...
class Test_H2_Statistical(unittest.TestCase):
    ...
    def test_cat(self):
        x = np.nanmean(list(map(t('cat'), self.h2)), axis=0)
    ...
    def test_prop_se(self):
        for h in [self.h2, self.h2_noint]:
            assert np.all(np.nanmean(list(map(t('prop_se'), h)), axis=0) - np.nanstd(list(map(t('prop'), h)),
                                                                                     axis=0) < 0.02)
...
class Test_Estimate(unittest.TestCase):
    ...
    def test_rg_M(self):
        ...
        with open(os.path.join(DIR, 'simulate_test/ldscore/oneld_onefile.l2.M_5_50'), 'rb') as f:
            args.M = f.read().decode().rstrip('\n')
        f.close()
